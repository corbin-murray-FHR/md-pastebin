...
- [x] T001 Verify current layout and typography settings in `src/components/Layout.tsx` and `src/components/MarkdownPreview.tsx`

## Phase 2: Foundational

> **Goal**: Establish baseline for improvements.

- [x] T002 Audit current contrast ratios and font sizes against research findings
...
- [x] T003 [US1] Update `src/components/Layout.tsx` to apply `max-w-[70ch]` (or similar) to the main content container
- [x] T004 [US1] Ensure content container is centered using `mx-auto` in `src/components/Layout.tsx`
- [x] T005 [US1] Verify and adjust padding for mobile responsiveness in `src/components/Layout.tsx`
...
- [x] T006 [US2] Update `src/components/MarkdownPreview.tsx` to enforce base font size of `1rem` (16px) using Tailwind classes
- [x] T007 [US2] Update `src/components/MarkdownPreview.tsx` to set body line-height to `1.5` (`leading-relaxed`)
- [x] T008 [US2] Update `src/components/MarkdownPreview.tsx` to set heading line-heights to `1.2`-`1.3` (`leading-tight`)
- [x] T009 [US2] Ensure proper vertical spacing between paragraphs and headings in `src/components/MarkdownPreview.tsx`
...
- [x] T010 [US3] Audit text colors in `src/index.css` (or theme config) and adjust for 4.5:1 contrast ratio if needed
- [x] T011 [US3] Verify layout behavior at 400% browser zoom and fix any horizontal scrolling issues

## Phase 6: Polish & Cross-Cutting Concerns

> **Goal**: Final verification and cleanup.

- [x] T012 Run final Lighthouse accessibility audit and address any remaining warnings
- [x] T013 Verify code block scrolling behavior (ensure no page scroll)
